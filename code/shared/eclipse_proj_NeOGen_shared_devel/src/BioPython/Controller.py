...
class GenePopController(object):
    ...
    # 2.1
    def test_ld(self, fname, boolPairOnly, dememorization=10000,
        ...
        def ld_func(self):
            ...
            if l == "":
                ...
                raise StopIteration
            toks = [x for x in l.split(" ") if x != ""]
            locus1, locus2 = toks[1], toks[2]
            try:
                #chi2, df, p = _gp_float(toks[3]), _gp_int(toks[4]), _gp_float(toks[5])
                p, se, switches = _gp_float(toks[3]), _gp_int(toks[4]), _gp_float(toks[5])
            except ValueError:
                return (locus1, locus2), None
            return (locus1, locus2), (p, se, switches)
