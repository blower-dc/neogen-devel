...
class EasyController(object):
    ...
    def test_hw_pop(self, pop_pos, test_type="probability"):
        if test_type=="deficiency":
            ...
            loci_res, hw_res = self._controller.test_pop_hz_prob(self._fname, ".P")
    ...
    def test_ld_all_pair(self, locus1='', locus2='', dememorization=10000,
        ...
        boolAllLocusPairs = False
        if locus1=='' and locus2=='':
            boolAllLocusPairs = True
        all_ld = self._controller.test_ld(self._fname, boolAllLocusPairs, dememorization, batches, iterations)[1]
    ...
    def get_heterozygosity_info_all_loci(self, pop_pos):
        """Returns the heterozygosity info for a certain locus on a population.
    ...
    def get_allele_frequency_all_loci(self, pop_pos):
        if len(self.__allele_frequency) == 0:
            geno_freqs = self._controller.calc_allele_genotype_freqs(self._fname)
            pop_iter, loc_iter = geno_freqs
            for locus_info in loc_iter:
                if locus_info[0] is None:
                    ...
                    self.__allele_frequency[locus_info[0]] = locus_info[1:]
        info = self.__allele_frequency
        dictAlleleFreqs = {}
        for key, value in info.items():
            pop_name, freqs, total = value[1][pop_pos]
            
            allele_freq = {}
            alleles = value[0]
            for i in range(len(alleles)):
                allele_freq[alleles[i]] = freqs[i]
            dictAlleleFreqs[key] = pop_name, allele_freq, total
            
        return dictAlleleFreqs
