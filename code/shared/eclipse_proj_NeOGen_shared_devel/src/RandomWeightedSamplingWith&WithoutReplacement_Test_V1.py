...
def random_weighted_sample_no_replacements_weighted_towards_lower_values(mapping, n):
    #Lower values are weighted highest WITHOUT replacement

    keys, values = zip(*mapping.items())
    total = functools.reduce(mul, (v + 1 for v in values))
    #weights = (total / mul(((v + 1) for v in values)))
    weights = (total / (v + 1) for v in values)
    
    #DEBUG_ON
    listWeights = []
    listWeights = list((total / (v + 1) for v in values))
    #print(str(sorted(((v + 1) for v in values), reverse=True)))
    print(str(list((v + 1) for v in values)))
    print(str(total))
    #print(str(sorted(listWeights)))
    print(str(listWeights))
    #DEBUG_OFF
...
def random_weighted_sample_no_replacements_weighted_towards_higher_values(mapping, n):
    #Higher values are weighted highest WITHOUT replacement
    keys, values = zip(*mapping.items())
    ...
    listWeights = []
    #listWeights = list((total / (v + 1) for v in values))
    listWeights = list(((v + 1) for v in values))
    #print(str(sorted(((v + 1) for v in values), reverse=True)))
    print(str(list((v + 1) for v in values)))
    #print(str(total))
    #print(str(sorted(listWeights)))
...
def weighted_sample(mapping, n):
    #Higher values are weighted highest WIT replacement
    keys, values = zip(*mapping.items())
    total = float(sum(w for w, v in values))
    i = 0
    w, v = items[0]
    while n:
