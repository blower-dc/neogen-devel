...
#define MyAppId "{{5FFE7385-3143-4C12-9F54-3873E9E0B57C}"
#define MyAppDirRoot "R:\WORK\NOG"
#define MyAppName "NeOGen-Power"
#define MyAppVersion "1.3.0.6.a1 (June 2019)"
...
Root: "HKCU"; Subkey: "Software\Microsoft\Windows NT\CurrentVersion\AppCompatFlags\Layers"; ValueType: string; ValueName: "{app}\{#MyAppExeName}"; ValueData: "~ DPIUNAWARE"; Flags: createvalueifdoesntexist uninsdeletevalue; MinVersion: 0,6.3
...
var
  ...
  if RegValueExists(HKEY_LOCAL_MACHINE,ExpandConstant('Software\Microsoft\Windows\CurrentVersion\Uninstall\{#MyAppId}_is1'), 'UninstallString') then  { Your App GUID/ID }
  begin
    V := MsgBox(ExpandConstant('Thanks for using NeOGen.' + #13#10 +  #13#10 +
      'You already have NeOGen installed.  If you continue with this setup it will overwrite all your previous NeOGen projects.' + #13#10 +  #13#10 +
      'You can backup your old data by archiving (zipping) the NeOGen install folder e.g. C:\NOG. and moving that to a safe location.' + #13#10 +  #13#10 + 
      'Are you sure you want to proceed to reinstall NeOGen and overwrite the existing installation?'), mbInformation, MB_YESNO); { Custom Message if App installed }
    if V = IDYES then
    begin
